...
function Instruments() {
    ...
    const getApiData = async () => {
        try {
            const res = await axios.get("http://localhost:8000/api/all-instupment")
                                    ...
                                    <div className="col-md-3" key={index}>
                                        <div className="category-card">
                                            <Link to={`/instrument-details/${instrument.instupmentName}`}>
                                                <img alt={instrument.instupmentName} className="img-fluid" src={`http://localhost:8000/${instrument.instupmentImage}`} width="250" height="230" />
                                            </Link>
