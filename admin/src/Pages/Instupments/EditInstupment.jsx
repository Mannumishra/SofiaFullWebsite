...
const EditInstupment = () => {
    ...
    useEffect(() => {
        const fetchInplant = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/api/single-instupment/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.put(`http://localhost:8000/api/update-instupment/${id}`, data, {
                headers: {
