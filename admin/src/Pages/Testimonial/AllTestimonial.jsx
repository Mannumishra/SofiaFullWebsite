...
const AllTestimonial = () => {
    ...
    useEffect(() => {
        ...
        const fetchTestimonials = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/all-textimonial'); // Adjust this endpoint as per your backend
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:8000/api/delete-textimonial/${id}`); // Adjust this endpoint as per your backend
                    ...
                    <tbody>
                        {testimonials.map((testimonial, index) => (
                            <tr key={testimonial._id}>
                                ...
                                <td>
                                    {testimonial.image ? (
                                        <img src={`http://localhost:8000/${testimonial.image}`} alt={testimonial.name} width="50" height="50" />
                                    ) : (
                                        "No Image"
                                    )}
                                </td>
