...
const AllCategory = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/get-all-category'); // Adjust the endpoint as necessary
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`http://localhost:8000/api/delete-category/${id}`); // Adjust the endpoint as necessary
                                ...
                                {currentCategories.length > 0 ? (
                                    currentCategories.map((category, index) => (
                                        <tr key={category._id}>
                                            ...
                                            <td>
                                                <img src={`http://localhost:8000/${category.categoryImage}`} alt={category.categoryName} style={{ width: '50px', height: '50px' }} />
                                            </td>
